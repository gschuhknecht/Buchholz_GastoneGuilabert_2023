...
def Find_STP_conductance_scaling_SHUFFLED_EPSPs(global_files,experiment_path, Theta_parameters,input_plotting):
    ...
    # HEADER: permute the EPSPs and associated PPRs randomly ===========================================================
    random_indices = np.random.permutation(range(270))
    rand_epsp_1 = epsp_1[random_indices]
    rand_ppr = ppr[random_indices]
    plt.scatter(range(270), rand_epsp_1,c = 'cornflowerblue')
    plt.scatter(range(270), rand_ppr, c = 'green')
    plt.title('EPSP-PPR shuffled')
    plt.savefig(experiment_path + 'EPSP_PPR_shuffled.eps',format='eps')
    plt.show()

    # Control: check that the PPR and EPSP mapping is still intact, the 2 printed values should be the same:
    test = 10
    print(rand_ppr[test])
    print(ppr[np.where(epsp_1 == rand_epsp_1[test])[0]])
    ...
    for i in range(n_inputs):
        Theta_index[i] = (np.abs(Theta_parameters[4] - rand_ppr[i])).argmin(axis = 0)    # find closes PPR value in Theta array

    # compute the maximum error of assigning a correct PPR, to be sure the method works:
    ppr_error = rand_ppr - Theta_parameters[4][Theta_index]      # index 4 = PPR values
    print(np.max(ppr_error))


    # Header: buils a conductance trace for each spike train ===========================================================
    ...
    # TODO Save numpy array and excel data =============================================================================
    np.save(experiment_path + 'Synaptic_conductance_scaling_SHUFFLED_EPSP.npy', Syn_conductance_list)
    np.save(experiment_path + 'SHUFFLED_EPSPs.npy', rand_epsp_1)

    excel_file = np.zeros([270,2])
    excel_file[:,0] = rand_epsp_1
    excel_file[:,1] =rand_ppr
    np.savetxt(experiment_path + 'EPSP_PPR_shuffled.csv', excel_file,
